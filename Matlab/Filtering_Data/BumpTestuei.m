...
Pi_Data = importdata('Pi_Test_3.txt');
...
Sensitivity = 0.44828125/1000; % Sensitivity = 32,000mg / 65,535 counts
...
% plot hz
for i = 1:(length(timeUEI)-1)
    hz1(i) = 1/(timeUEI(i+1)-timeUEI(i));
end
...
L = length(timeUEI);             % Length of signal

Y = fft(detrend(accelyUEI));
Z = fft(detrend(accelzUEI));
P2 = abs(Y/L);
P1 = P2(1:L/2+1);
...
y = lp10(detrend(accelyUEI));
z = lp10(detrend(accelzUEI));
...
yindexmax = find(max(y) == y);
ymax = y(yindexmax);
tmax = timeUEI(yindexmax);
strmax = ['ymax = ',num2str(round(xmax,2)),' G\rightarrow'];
text(tmax,ymax,strmax,'HorizontalAlignment','right');
...
Lf = length(timeUEI);             % Length of signal
