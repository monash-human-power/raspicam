...
client.loop_start()
client.subscribe("/Accelerometer/DataStream")

# Value to initialise startup angle
averageX = 0
averageY = 0
averageZ = 0
averageLoop = 100
...
try:
	...
	if start == True:
		...
		# Initialising the starting orientation of the accelerometer
		if increment < averageLoop:
			averageX += CFangleX
			averageY += CFangleY
			averageZ += tiltCompensatedHeading
			increment += 1
		elif increment == averageLoop:
			averageX /= averageLoop
			averageY /= averageLoop
			averageZ /= averageLoop
			increment += 1

	        # Publish data to Node Red server
		Xangle = round(CFangleX-averageX,2)
		Yangle = round(CFangleY-averageY,2)
		Zangle = round(tiltCompensatedHeading-averageZ,2)

		if Zangle < 0:
        	                Zangle += 360

	        client.publish("/Accelerometer/Pitch",Xangle)
        	client.publish("/Accelerometer/Roll",Yangle)
	        client.publish("/Accelerometer/Heading", Zangle)
		print(str(tiltCompensatedHeading) + '\t' + str(Zangle))
