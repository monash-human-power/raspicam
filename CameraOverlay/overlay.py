...
def on_message(client, userdata, msg):
	...
	if msg.topic == "power_model/recommended_SP":
		...
		if total_time >= update_time:
			...
			draw.text((50, (top_box_height - top_text_height) / 2 + 8),
			          "{:0>2}:{:0>2}".format(int(minutes), int(seconds)), font=top_text_font, fill='white')
			...
			if DAS_DATA["power"] != 0:
				...
				draw.text((600, (top_box_height - top_text_height) / 2 + 8), "{0}".format(round(rec_power, 0)),
				          font=top_text_font, fill='white')
				# Display power (no colour change)
				draw.text((WIDTH / 2 - 90, HEIGHT - bottom_text_height), "P:{0}".format(round(power, 2)),
				          font=bottom_text_font, fill='red')
			...
			if int(parsed_data["gps"]) == 1:
				if DAS_DATA["gps_speed"] != 0:
					...
					draw.text((890, (top_box_height - top_text_height) / 2 + 8), "{0}".format(round(pred_max_speed, 2)),
					          font=top_text_font, fill='white')

					# Actual speed (no colour change)
					speed = DAS_DATA["gps_speed"] / DAS_DATA["count"]
					speed_text = "{0}".format(round(speed, 2))
					draw.text((WIDTH / 2 - 90, HEIGHT - bottom_text_height * 2 - 30), "S:{0}".format(round(speed, 2)),
					          font=bottom_text_font, fill='red')

					# Actual max speed
					actual_max(speed)
					draw.text((1120, (top_box_height - top_text_height) / 2 + 8), "{0}".format(int(MAX_SPEED)),
					          font=top_text_font, fill='white')

			# Display zone distance left (bugged)
			if POWER_MODEL_DATA["zdist"] != 0:
				zdist_left = POWER_MODEL_DATA["zdist"]
				draw.text((360, (top_box_height - top_text_height) / 2 + 8), "{0}".format(int(zdist_left)),
				          font=top_text_font, fill='white')
...
class Overlay(ABC):
	...
	def connect(self, ip, port):
		...
		camera.start_preview(fullscreen=False, window=(0, -20, self.width, self.height))
		client.loop_start()
		while True:
			time.sleep(1)
